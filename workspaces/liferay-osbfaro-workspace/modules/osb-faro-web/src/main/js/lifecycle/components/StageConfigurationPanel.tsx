...
const StageConfigurationPanel: React.FC<IStageConfigurationPanelProps> = ({
	...
	const renderValueInput = () => {
		...
		const type =
			resolvedType === 'Number' || resolvedType === 'Duration'
				? 'number'
				: 'text';
		...
		>
			<Panel.Body>
				...
				<div className="align-items-center c-gap-2 d-flex mb-4">
					...
					{CONDITION_STEPS.map((step, stepIndex) => {
						const previousStep = CONDITION_STEPS[stepIndex - 1];

						if (previousStep && !value[previousStep.key]) {
							return null;
						}

						const isFieldStep = step.key === 'field';
				...
				<div className="align-items-center c-gap-2 d-flex">
					...
					<ClayInput
						...
						onChange={(
							...
							onChange({
								...value,
								maxTimeDays: Number(event.target.value),
							})
						}
						sizing="sm"
						type="number"
